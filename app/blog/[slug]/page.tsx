...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  const post = await getPublishedPost('IN', params.slug)
  if (!post) return { title: 'Article not found | Ambel POS' }
  const canonical = `https://in.ambelpos.com/blog/${post.slug}`
...
export default async function Page({ params }: { params: { slug: string } }) {
  const post = await getPublishedPost('IN', params.slug)
  if (!post) notFound()
  const url = `https://in.ambelpos.com/blog/${post.slug}`
  ...
  return <><SiteHeader /><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, '\\u003c') }} /><BlogArticle post={post} region="IN" /><SiteFooter /></>
}
