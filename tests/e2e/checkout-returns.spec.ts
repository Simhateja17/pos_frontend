...
test('checkout retains correction state until the sale API confirms a charge', async ({ authenticatedPage }, testInfo) => {
  ...
  await authenticatedPage.getByRole('button', { name: 'Enter cash received for change' }).click()
  await authenticatedPage.getByLabel('Cash received from customer').fill('1000.00')
