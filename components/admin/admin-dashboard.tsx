...
function BusinessesView({ context, onError, focus }: { context: AdminContext; onError: (message: string | null) => void; focus?: 'users' | 'subscriptions' }) {
  ...
  return <>
    ...
    {detail && <>
      ...
      {focus === 'subscriptions' && context.admin.role === 'platform_owner' && <section className={styles.card} style={{ marginTop: 16 }}><div className={styles.cardHeader}><div><div className={styles.cardTitle}>Private negotiated plan</div><div className={styles.muted}>Creates an immutable Razorpay Plan; the owner must authorise payment.</div></div><Badge value="owner approval required" /></div><div className={styles.cardBody}><form onSubmit={createOffer}><div className={styles.formRow}><select className={styles.select} value={offer.plan} onChange={(event) => setOffer((current) => ({ ...current, plan: event.target.value as typeof current.plan }))}><option value="starter">Starter</option><option value="growth">Growth</option><option value="pro">Pro</option></select><select className={styles.select} value={offer.cycle} onChange={(event) => setOffer((current) => ({ ...current, cycle: event.target.value as typeof current.cycle }))}><option value="monthly">Monthly</option><option value="annual">Annual</option></select><input className={styles.input} required type="number" min="1" step="0.01" value={offer.price} onChange={(event) => setOffer((current) => ({ ...current, price: event.target.value }))} placeholder="Price before tax" /><input className={styles.input} required type="number" min="0" value={offer.trialDays} onChange={(event) => setOffer((current) => ({ ...current, trialDays: event.target.value }))} placeholder="Trial days" /></div><div className={styles.formRow}><input className={styles.input} required type="number" min="1" value={offer.locations} onChange={(event) => setOffer((current) => ({ ...current, locations: event.target.value }))} placeholder="Locations included" /><input className={styles.input} required type="number" min="1" value={offer.registers} onChange={(event) => setOffer((current) => ({ ...current, registers: event.target.value }))} placeholder="Registers included" /><input className={styles.input} required type="number" min="1" value={offer.users} onChange={(event) => setOffer((current) => ({ ...current, users: event.target.value }))} placeholder="Users included" /><input className={styles.input} required type="datetime-local" value={offer.activation} onChange={(event) => setOffer((current) => ({ ...current, activation: event.target.value }))} aria-label="Latest activation" /></div><div className={styles.formRow}><input className={styles.input} required type="number" min="0" step="0.01" value={offer.locationAddon} onChange={(event) => setOffer((current) => ({ ...current, locationAddon: event.target.value }))} placeholder="Extra location price" /><input className={styles.input} required type="number" min="0" step="0.01" value={offer.registerAddon} onChange={(event) => setOffer((current) => ({ ...current, registerAddon: event.target.value }))} placeholder="Extra register price" /><input className={styles.input} required type="number" min="0" step="0.01" value={offer.userAddon} onChange={(event) => setOffer((current) => ({ ...current, userAddon: event.target.value }))} placeholder="Extra user price" /></div><div className={styles.formRow}><input className={styles.input} value={offer.reference} onChange={(event) => setOffer((current) => ({ ...current, reference: event.target.value }))} placeholder="Sales reference (optional)" /><input className={styles.input} required value={offer.reason} onChange={(event) => setOffer((current) => ({ ...current, reason: event.target.value }))} placeholder="Internal negotiation reason" /><button className={`${styles.button} ${styles.primary}`} disabled={busy} type="submit">Create private offer</button></div><div className={styles.notice} style={{ marginTop: 12 }}>The entered plan price is before tax. Add-on rates are stored with the offer. The owner sees the base amount, tax, and exact recurring total before Razorpay opens.</div></form></div></section>}
    </>}
  </>
}
