...
function moneyLabel(amountMinor: unknown, currency: unknown) {
  const amount = Number(amountMinor)
  if (!Number.isFinite(amount)) return '—'
  try {
    return new Intl.NumberFormat(undefined, { style: 'currency', currency: String(currency || 'INR') }).format(amount / 100)
  } catch {
    return `${String(currency || '')} ${(amount / 100).toFixed(2)}`.trim()
  }
}
...
function baseAmountFromTaxInclusive(totalMajor: string, taxRateBps: number) {
  const totalMinor = Math.round(Number(totalMajor) * 100)
  if (!Number.isFinite(totalMinor) || totalMinor < 0) return null
  const estimate = Math.round(totalMinor * 10_000 / (10_000 + taxRateBps))
  for (let candidate = Math.max(0, estimate - 2); candidate <= estimate + 2; candidate += 1) {
    if (candidate + Math.round(candidate * taxRateBps / 10_000) === totalMinor) return candidate
  }
  return estimate
}

function majorAmount(minor: number | null) {
...
function BusinessesView({ context, onError, focus }: { context: AdminContext; onError: (message: string | null) => void; focus?: 'users' | 'subscriptions' }) {
  ...
  const [offer, setOffer] = useState({ plan: 'starter' as 'starter' | 'growth' | 'pro', cycle: 'monthly' as 'monthly' | 'annual', totalPrice: '', price: '', locations: '2', registers: '3', users: '5', locationAddon: '299', registerAddon: '199', userAddon: '99', trialDays: '14', trialUnit: 'days' as 'minutes' | 'hours' | 'days', activation: '', reason: '', reference: '' })
  const taxRateBps = context.taxRateBps ?? (context.admin.region === 'IN' ? 1_800 : 0)
  ...
  async function createOffer(event: FormEvent) {
    event.preventDefault()
    if (!detail || context.admin.role !== 'platform_owner') return
    if (!window.confirm('Create this Razorpay Plan and show the negotiated offer to the business owner?')) return
    setBusy(true); onError(null)
    try {
      const multiplier = context.admin.region === 'IN' ? 100 : 100
      await createPrivateBillingOffer({
        tenantId: detail.tenant.id, basePlanKey: offer.plan, billingCycle: offer.cycle,
        negotiatedBaseAmountMinor: Math.round(Number(offer.price) * multiplier),
        includedLocations: Number(offer.locations), includedRegisters: Number(offer.registers), includedUsers: Number(offer.users),
      ...
      setDetail(await getAdminTenant(detail.tenant.id))
    } catch (cause) { onError(errorMessage(cause)) } finally { setBusy(false) }
  }

  function updateTotalPrice(totalPrice: string) {
    const baseMinor = baseAmountFromTaxInclusive(totalPrice, taxRateBps)
    setOffer((current) => ({ ...current, totalPrice, price: majorAmount(baseMinor) }))
  }

  return <>
    {!detail && <><h1 className={styles.heading}>{focus === 'users' ? 'Merchant users' : focus === 'subscriptions' ? 'Subscriptions' : 'Businesses'}</h1>
    ...
    {detail && <>
      ...
      {focus === 'subscriptions' && context.admin.role === 'platform_owner' && <div className={styles.notice} style={{ marginBottom: 16 }}>For a free trial, enter a number and choose Minutes, Hours, or Days in the form below. The activation deadline is the latest time the owner can accept the offer.</div>}
      ...
      <section className={`${styles.grid} ${styles.columns}`} style={{ marginTop: 16 }}>
        ...
        <section className={styles.card}><div className={styles.cardHeader}><div className={styles.cardTitle}>Merchant users</div></div><div className={styles.cardBody}><div className={styles.list}>{detail.users.map((user) => <div className={styles.listItem} key={user.id}><div className={styles.split}><span className={styles.listMain}>{user.name}</span><Badge value={user.isActive ? user.role : 'suspended'} /></div><div className={styles.listMeta}>{user.email || 'No email account'} · {user.id}</div></div>)}</div>{detail.users.length === 0 && <div className={styles.empty}>No merchant users.</div>}</div></section>
      </section>
      ...
      <section className={`${styles.grid} ${styles.columns}`} style={{ marginTop: 16 }}>
        <section className={styles.card}><div className={styles.cardHeader}><div className={styles.cardTitle}>Entitlement overrides</div></div><div className={styles.cardBody}><div className={styles.list}>{detail.entitlements.map((item) => <div className={styles.listItem} key={String(item.id)}><div className={styles.split}><span className={styles.listMain}>{String(item.entitlementKey)} = {String(item.overrideValue)}</span><Badge value={item.revokedAt ? 'revoked' : 'active'} /></div><div className={styles.listMeta}>{String(item.justification ?? '—')} · ticket {String(item.ticketId ?? '—')} · expires {dateLabel(item.expiresAt)}</div></div>)}</div>{detail.entitlements.length === 0 && <div className={styles.empty}>No entitlement overrides.</div>}</div></section>
        <section className={styles.card}><div className={styles.cardHeader}><div className={styles.cardTitle}>Operational failures</div></div><div className={styles.cardBody}><div className={styles.list}>{styleslistItem(detail.operationalFailures.emails.length, 'Email sends')}{styleslistItem(detail.operationalFailures.imports.length, 'Imports')}{styleslistItem(detail.operationalFailures.forecasts.length, 'Forecast runs')}</div></div></section>
      </section>
      ...
      {focus === 'subscriptions' && context.admin.role === 'platform_owner' && <section className={styles.card} style={{ marginTop: 16 }}><div className={styles.cardHeader}><div><div className={styles.cardTitle}>Private negotiated plan</div><div className={styles.muted}>Creates an immutable Razorpay Plan; the owner must authorise payment.</div></div><Badge value="owner approval required" /></div><div className={styles.cardBody}><form onSubmit={createOffer}><div className={styles.formRow}><select className={styles.select} value={offer.plan} onChange={(event) => setOffer((current) => ({ ...current, plan: event.target.value as typeof current.plan }))}><option value="starter">Starter</option><option value="growth">Growth</option><option value="pro">Pro</option></select><select className={styles.select} value={offer.cycle} onChange={(event) => setOffer((current) => ({ ...current, cycle: event.target.value as typeof current.cycle }))}><option value="monthly">Monthly</option><option value="annual">Annual</option></select><input className={styles.input} required type="number" min="0.01" step="0.01" value={offer.totalPrice} onChange={(event) => updateTotalPrice(event.target.value)} placeholder="Total amount incl. tax" aria-label="Total amount including tax" /><input className={styles.input} required type="number" value={offer.price} readOnly aria-label="Pre-tax amount (auto-filled)" placeholder="Pre-tax amount (auto-filled)" /><div className={styles.trialControl}><input className={styles.input} required type="number" min="0" value={offer.trialDays} onChange={(event) => setOffer((current) => ({ ...current, trialDays: event.target.value }))} placeholder="Trial duration" aria-label="Trial duration" /><select className={`${styles.select} ${styles.trialUnit}`} aria-label="Trial duration unit" value={offer.trialUnit} onChange={(event) => setOffer((current) => ({ ...current, trialUnit: event.target.value as typeof current.trialUnit }))}><option value="minutes">Minutes</option><option value="hours">Hours</option><option value="days">Days</option></select></div></div><div className={styles.formRow}><input className={styles.input} required type="number" min="1" value={offer.locations} onChange={(event) => setOffer((current) => ({ ...current, locations: event.target.value }))} placeholder="Locations included" /><input className={styles.input} required type="number" min="1" value={offer.registers} onChange={(event) => setOffer((current) => ({ ...current, registers: event.target.value }))} placeholder="Registers included" /><input className={styles.input} required type="number" min="1" value={offer.users} onChange={(event) => setOffer((current) => ({ ...current, users: event.target.value }))} placeholder="Users included" /><input className={styles.input} required type="datetime-local" value={offer.activation} onChange={(event) => setOffer((current) => ({ ...current, activation: event.target.value }))} aria-label="Latest activation" /></div><div className={styles.formRow}><input className={styles.input} required type="number" min="0" step="0.01" value={offer.locationAddon} onChange={(event) => setOffer((current) => ({ ...current, locationAddon: event.target.value }))} placeholder="Extra location price" /><input className={styles.input} required type="number" min="0" step="0.01" value={offer.registerAddon} onChange={(event) => setOffer((current) => ({ ...current, registerAddon: event.target.value }))} placeholder="Extra register price" /><input className={styles.input} required type="number" min="0" step="0.01" value={offer.userAddon} onChange={(event) => setOffer((current) => ({ ...current, userAddon: event.target.value }))} placeholder="Extra user price" /></div><div className={styles.formRow}><input className={styles.input} value={offer.reference} onChange={(event) => setOffer((current) => ({ ...current, reference: event.target.value }))} placeholder="Sales reference (optional)" /><input className={styles.input} required value={offer.reason} onChange={(event) => setOffer((current) => ({ ...current, reason: event.target.value }))} placeholder="Internal negotiation reason" /><button className={`${styles.button} ${styles.primary}`} disabled={busy} type="submit">Create private offer</button></div><div className={styles.notice} style={{ marginTop: 12 }}>Enter the total amount including tax. The pre-tax amount is filled automatically using the regional tax rate ({(taxRateBps / 100).toFixed(2)}%). For a free trial, choose a number and unit above (for example, 30 Minutes, 2 Hours, or 14 Days). Add-on rates are stored with the offer, and the owner sees the same breakdown before Razorpay opens.</div></form></div></section>}
    </>}
  </>
}
