...
export function OrdersView() {
  ...
  const load = useCallback(async (nextCursor?: string) => {
    ...
    try { setData(await getAuthenticatedSales({ search: search || undefined, cursor: nextCursor, limit: 25 })); setCursor(nextCursor) }
  ...
  return <main className="p-5 md:p-8">
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b p-5">
        <p className="text-sm text-slate-500">Use invoice UUID or customer name to find a completed sale.</p>
