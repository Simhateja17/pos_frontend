  ...
  return <>
    ...
    <form id="new-product-form" onSubmit={handleSubmit}>
      ...
      <Card><CardHead title="Product" sub="Type a product name to search Ambel’s catalogue, or continue with your own item" /><CardPad>
        ...
        <details style={{ marginTop: 12 }}><summary style={{ cursor: 'pointer', fontSize: 13, fontWeight: 650 }}><ChevronDown size={14} style={{ verticalAlign: 'middle' }} /> More product details (optional)</summary><div style={{ ...grid(240), marginTop: 10 }}><Fld id="product-description" label="Description"><textarea id="product-description" value={description} onChange={(event) => setDescription(event.target.value)} rows={3} /></Fld><Fld id="product-notes" label="Internal notes"><textarea id="product-notes" value={internalNotes} onChange={(event) => setInternalNotes(event.target.value)} rows={3} /></Fld></div></details>
      </CardPad></Card>
      <Card><CardHead title="Variants" sub="Add one row for each pack, size, colour, or other sellable version" right={<button type="button" className="btn btn-sm" onClick={() => setRows((current) => [...current, { ...EMPTY_ROW }])}><Plus size={14} /> Add variant</button>} /><CardPad style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        {rows.map((row, index) => { const suffix = unitSuffix(row.unitOfMeasure); const fractional = allowsFractionalQuantity(row.unitOfMeasure); return <div key={index} style={{ border: '1px solid var(--line)', borderRadius: 12, padding: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}><strong style={{ fontSize: 12, color: 'var(--muted)' }}>VARIANT {index + 1}</strong>{rows.length > 1 && <button type="button" className="btn btn-sm btn-ghost" onClick={() => setRows((current) => current.filter((_, rowIndex) => rowIndex !== index))}><Trash2 size={14} /> Remove</button>}</div>
          <div style={grid()}>{india ? <Fld id={`variant-${index}-mrp`} label="MRP"><input id={`variant-${index}-mrp`} type="number" min="0" step="0.01" required value={row.mrp} onChange={(event) => changeMrp(index, event.target.value)} placeholder="Required" /></Fld> : <Fld id={`variant-${index}-list-price`} label="List / compare-at price (optional)"><input id={`variant-${index}-list-price`} type="number" min="0" step="0.01" value={row.listPrice} onChange={(event) => setRow(index, { listPrice: event.target.value })} /></Fld>}
            ...
            <Fld id={`variant-${index}-unit`} label="Sold by"><select id={`variant-${index}-unit`} value={row.unitOfMeasure} onChange={(event) => setRow(index, { unitOfMeasure: event.target.value as Unit })}>{UNITS.map((unit) => <option key={unit.value} value={unit.value}>{unit.label}</option>)}</select></Fld></div>
          <div style={{ ...grid(220), marginTop: 10 }}><Fld id={`variant-${index}-track`} label="Inventory"><label style={{ display: 'flex', gap: 8, alignItems: 'center', minHeight: 38 }}><input type="checkbox" checked={row.trackInventory} onChange={(event) => setRow(index, { trackInventory: event.target.checked, allowNegativeStock: event.target.checked ? row.allowNegativeStock : false })} /> Track inventory</label></Fld>
            {row.trackInventory && <Fld id={`variant-${index}-negative`} label="Stock rule"><label style={{ display: 'flex', gap: 8, alignItems: 'center', minHeight: 38 }}><input type="checkbox" checked={row.allowNegativeStock} onChange={(event) => setRow(index, { allowNegativeStock: event.target.checked })} /> Allow sales below zero</label></Fld>}
            {row.trackInventory && <Fld id={`variant-${index}-opening`} label={suffix ? `Opening stock (${suffix})` : 'Opening stock'}><input id={`variant-${index}-opening`} type="number" min="0" step={fractional ? '0.001' : '1'} value={row.openingStock} onChange={(event) => setRow(index, { openingStock: event.target.value })} placeholder="0" /></Fld>}</div>
          <details style={{ marginTop: 12 }}><summary style={{ cursor: 'pointer', fontSize: 13, fontWeight: 650 }}><ChevronDown size={14} style={{ verticalAlign: 'middle' }} /> More variant details (optional)</summary><div style={{ ...grid(), marginTop: 10 }}>
            ...
            <Fld id={`variant-${index}-barcode`} label={<span style={{ display: 'inline-flex', gap: 5, alignItems: 'center' }}><ScanLine size={13} /> Barcode</span>}><input id={`variant-${index}-barcode`} inputMode="numeric" maxLength={14} value={row.barcode} onChange={(event) => setRow(index, { barcode: event.target.value })} placeholder="Scan or type" /></Fld>
            ...
            <Fld id={`variant-${index}-reorder`} label="Reorder at"><input id={`variant-${index}-reorder`} type="number" min="0" step={fractional ? '0.001' : '1'} value={row.reorderThreshold} onChange={(event) => setRow(index, { reorderThreshold: event.target.value })} placeholder="Defaults to 4" /></Fld>
