...
function formatStamp(iso: string, locale: string, timeZone?: string): string {
      ...
      <div className="sr-only">
        <div ref={contentRef} className={`receipt-print receipt-print--${paperWidth}`}>
          ...
          <div className="receipt-meta">
            ...
            <span>{formatStamp(sale.createdAt, pack.locale, pack.timeZone)}</span>
          </div>
          <div className="receipt-rule" />
          <div className="receipt-cols">
            <span>ITEM</span>
            <span>AMOUNT ({pack.currency})</span>
          </div>
          ...
          {sale.lines.map((line) => (
            <div key={line.variantId} className="receipt-item">
              ...
              <div className="receipt-item-calc">
                <span>
                  {line.quantity} × {money(line.unitPrice, formatMoney)}
                </span>
                <span className="receipt-amount">{money(line.lineTotal, formatMoney)}</span>
              </div>
            </div>
          ))}
          <div className="receipt-rule-dashed" />
          <div className="receipt-row">
            <span>Subtotal</span>
            <span className="receipt-amount">{money(sale.subtotal, formatMoney)}</span>
          </div>
          <div className="receipt-row">
            <span>Discount</span>
            <span className="receipt-amount">-{money(sale.discountAmount, formatMoney)}</span>
          </div>
          <div className="receipt-row">
            <span>GST</span>
            <span className="receipt-amount">{money(sale.taxAmount, formatMoney)}</span>
          </div>
          <div className="receipt-rule" />
          <div className="receipt-total">
            <span>TOTAL</span>
            <span className="receipt-amount">{money(sale.totalAmount, formatMoney)}</span>
          </div>
          <div className="receipt-rule-strong" />
          {payments.map((payment, index) => (
            <div key={`${payment.method}-${index}`} className="receipt-row">
              ...
              <span className="receipt-amount">{money(payment.amount, formatMoney)}</span>
            </div>
          ))}
          {sale.cashReceived ? (
            <>
              <div className="receipt-row"><span>Cash received</span><span className="receipt-amount">{money(sale.cashReceived, formatMoney)}</span></div>
              <div className="receipt-row"><span>Change returned</span><span className="receipt-amount">{money(sale.changeDue, formatMoney)}</span></div>
            </>
          ) : null}
          <div className="receipt-rule-dashed" />
